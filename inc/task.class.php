...
JAVASCRIPT;
   ...
   static public function postShowItem($params) {
      ...
      if ($row=$req->next()) {
         ...
         $div="<div id='timer$rand'>".__("Timer started on", 'actualtime')." <a href='/front/ticket.form.php?id=".$ticket_id."'>".__("Ticket")." ".$ticket_id."</a> -> <span>".HTML::timestampToString($seconds)."</span></div>";
         $script=<<<JAVASCRIPT
            $(document).ready(function(){
               if ($("#timer{$rand}").length) {
                  $("#timer{$rand}").remove();
